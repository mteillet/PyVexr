...
std::vector<py::array_t<float>> loadExrChan(const std::string& filename, const std::vector<std::string>& selectedChannel){
	// Open the .exr file 
	Imf::MultiPartInputFile exr_file(filename.c_str());

	// img height and width
	Header header = exr_file.header(0);
	...
	int pixel_base = exr_file.header(0).dataWindow().min.y* (exr_file.header(0).dataWindow().size().x+1) + exr_file.header(0).dataWindow().min.x;
	...
	py::array_t<float> channelG = channelR
	py::array_t<float> channelB = channelR
